<script lang="ts">
	...
	export function setTooltip(tooltip: HTMLElement | string, customStyles: Partial<CSSStyleDeclaration> = {}) {
		try {
			reactTooltip = tooltip as HTMLElement | string;
			if (typeof tooltip === 'string' || tooltip instanceof String) {
				// console.log('tooltip is string');
				tooltipEl = document.createElement('div') as HTMLDivElement;
				// console.log('created tooltipEl', tooltipEl);
				...
				// console.log('final tooltipEl', tooltipEl);
				tooltipRect = tooltipEl.getBoundingClientRect();
			} else {
				// console.log('tooltip is HTMLElement');
				tooltipRect = (tooltip as HTMLElement).getBoundingClientRect();
				tooltipEl = tooltip as HTMLElement;
			}
			tooltipEl.style.cssText += 'opacity:0; transform:scale(0); transition: opacity 0.5s ease, transform 0.8s ease;';

			// console.log('setTooltip tooltipEl', tooltipEl);
	...
	function positionTooltip(tooltipEl: HTMLElement, stick?: TStick) {
		//console.log('positionTooltip tooltipEl', tooltipEl);
		try {
			// top-left position of hovering element
			...
			if (stick) {
				switch (stick) {
					...
					case 'below':
						...
						//console.log('position below y=', y);
						break;
					default:
						y -= 6;
						break;
			...
			const msg = err instanceof Error ? err.message : String(err);
			console.log('catch positionTooltip', msg);
		}
	}
	export function hideTooltip() {
		//console.log('hideTooltip tooltipEl', tooltipEl);
	...
	export function showTooltip(
		...
		try {
			...
			const hoveringEl = document.elementFromPoint(e.clientX, e.clientY) as HTMLElement;
