<script lang="ts">
	// TODO import { vscode } from '$lib/utils/event-handler.browser';
	...
	let isButtonDisabled: boolean = $derived(!(db.name && db.owner && db.password));
	const inputStyle =
		'display: block;width: 18rem;height: 1rem !important;margin: 8px 0 10px 0;padding: 6px 0 8px 1rem;border-radius: 4px;outline: none;';
	...
	// function approveAll() {
	// 	const container = document.querySelector('.approval-section') as HTMLDivElement;
	...
	// TODO added for the test app 'event-handler'
	let isActive = $state(false);
</script>

<!-- TODO  added for the test app 'event-handler'-->
<button onclick={() => (isActive = !isActive)}>toggle isActive</button>
...
{/snippet}
{#if isActive}
	<div class="page-info" style="position:absolute;top:5px;left:1rem;z-index:200;">
		{@render pagePurpose()}
	</div>
{/if}

<div class="buttons-row">
	<details style="position:relative;z-index:200;">
		<summary class="summary">Parameters for Creating Database </summary>
		<div
			class="dbname-block"
			style="border: 1px solid gray;width:19rem;padding:0.5rem;
		...
		>
			<label>
				Database Name
				<br /><input bind:value={db.name} type="text" placeholder="avoid dashes in db-name" />
			</label>
			<label for="dbOwnerId">
				Database Owner
				<br /><input bind:value={db.owner} id="dbOwnerId" type="text" />
			</label>
			<label>
				Owner's Password
				<br /><input bind:value={db.password} type="password" style={inputStyle} />
			</label>
			<label>
				Host Name
				<br /><input bind:value={db.host} type="text" />
			</label>
			<label>
				Communication Port
	...
	<!--TODO  onclick={startPrismaInstall} -->
	<button
	...
	<!-- TODO <button class="button-close" onclick={closetheApp}>close</button> -->
...
<div class="grid-container">
	<div class="left-column">
		<label class="dependencies-label">
			<input type="checkbox" bind:checked={useOnlyBuiltDependencies} style="display:inline-block;" />
	...
	<div class="approval-section">
		...
		<ul>
			{#each approvalPackages as pkg (pkg)}
				<li>
					<!-- TODO <button onclick={(e: MouseEvent) => approvePackage(e, pkg)}>{pkg}</button> -->
					<button>{pkg}</button>
				</li>
			{/each}
		</ul>
		<!-- TODO <button onclick={approveAll}>Approve All</button> -->
	...
	.page-info {
		...
		pre {
			...
			span {
				...
				text-shadow: 3px 2px 6px var(--pre-title-shadow);
			}
		}
	}
	div,
