<script lang="ts">
	...
	let buttonNotAllowed = $derived(Object.keys(selectedModels).length === 0) as boolean;
	...
	onMount(() => {
		try {
			...
			// 			// console.log('[OrmThree] getting models', pload.models);
			// 			// models = pload.models;
			// 			// userRoles = Object.keys(Object.values(pload.enums)[0] as TEnum).filter(Boolean);
			...
			console.log('[OrmThree] onMount try/catch', err);
		}
		if (models) {
			isLoading = false;
		}
		...
		<div class="embellishments" aria-hidden={true}>
			{#each ['CRInput', 'CRSpinner', 'CRActivity', 'CRTooltip', 'CRSummaryDetails'] as comp (comp)}
				<div class="checkbox-item">
					<label for={comp}
						><input id={comp} type="checkbox" value={comp} bind:group={crComponents} />
						{comp} component
					</label>
				</div>
