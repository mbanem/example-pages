<script lang="ts">
	...
	async function deleteModel(e: MouseEvent, modelName: string) {
		CRShowTooltip.showTooltip(e, `Model "${modelName}" to be removed.`, 'right', {
		...
		if (confirmed) {
			...
			if (models[modelName]) {
				CRShowTooltip.showTooltip(e, `Model "${modelName}" has been removed.`, 'left', {
					backgroundColor: 'black',
					color: 'lightgreem',
				});
			} else {
				CRShowTooltip.showTooltip(e, `Model "${modelName}" is removed.`, 'over', {
					backgroundColor: 'navy',
					color: 'white',
				});
			}
...
<!-- no display just a Tooltip utils with markup -->
<Tooltip bind:this={CRShowTooltip} />
